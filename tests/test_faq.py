...
class TestFaq:
    ...
    @pytest.mark.parametrize(
        ...
        MainPageMethods.get_qa_data()
        )
    def test_faq_question_and_answer(self, main_page_methods, question_locator, answer_locator, expected_text):
        ...
        with allure.step(f"Кликнуть на вопрос и проверить ответ содержит '{expected_text}'"):
            main_page_methods.click_question(question_locator)
            answer_text = main_page_methods.get_answer_text(answer_locator)
    ...
    def test_all_faq_items_have_answers(self, main_page_methods):
        ...
        with allure.step("Собрать все вопросы и ответы"):
            faq_dict = main_page_methods.qa_dictionary()
